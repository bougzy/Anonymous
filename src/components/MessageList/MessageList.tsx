...
import DeleteAllMessagesButton from '../DeleteAllMessagesButton/DeleteAllMessagesButton';
import ShareMessageButton from '../ShareMessageButton/ShareMessageButton';
                ...
                <div className="flex flex-col items-start">
                    {messages.map((msg, index) => (
                        <div key={index} className="bg-gray-700 text-white mb-2 rounded-md flex justify-between items-center w-full max-w-md p-2">
                            <div className="flex items-center space-x-4 w-full">
                                <p className="flex-1">{msg.message}</p>
                                <DeleteMessageButton onDelete={() => deleteMessage(index)} />
                                <ShareMessageButton onShare={() => shareMessage(msg.message)} />
                            </div>
                        </div>
                    ))}
                    <DeleteAllMessagesButton onDeleteAll={deleteAllMessages} />
                </div>
            ) : (
                <p className="text-white">No messages available.</p>
            )}
